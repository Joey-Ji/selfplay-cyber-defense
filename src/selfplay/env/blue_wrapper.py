...
from selfplay.env.agents import BLineAgent, MeanderAgent
from selfplay.env.cage import HOSTS, SimplifiedCAGE

_RED_AGENTS: dict[str, type] = {
...
def make_red_agent(name: str, sim: SimplifiedCAGE):
    """Return a scripted red agent by name."""
    cls = _RED_AGENTS.get(name)
    if cls is None:
        raise ValueError(
            f"Unknown red agent: {name!r}. Available: {sorted(_RED_AGENTS)}"
        )
    return cls()
