"""Scripted agents for the CAGE environment."""
...
class BaseAgent(ABC):
    """Base class for scripted agents in the CAGE environment."""
    ...
    def get_action(self, observation: np.ndarray) -> np.ndarray:
        ...
        Args:
            observation: Array of shape ``(batch, obs_dim)``.

        Returns:
            Integer action array of shape ``(batch, 1)``.
        """

    def reset(self) -> None:
        """Reset any internal episode state. Override in stateful agents."""


class BLineAgent(BaseAgent):
    """B-line red agent: follows a fixed attack path through the network."""
...
class MeanderAgent(BaseAgent):
    """Meander red agent: explores the network opportunistically."""
...
class ReactRestoreAgent(BaseAgent):
    """React-restore blue agent: restores compromised hosts."""

    def __init__(self) -> None:
        self.num_hosts = 13
        self.host_list: np.ndarray | None = None
...
class BlueSleepAgent(BaseAgent):
    """Inactive blue agent — always selects action 0 (sleep)."""

    def get_action(self, observation: np.ndarray) -> np.ndarray:
        return np.zeros((observation.shape[0], 1)).astype(int)


class RestoreDecoysAgent(ReactRestoreAgent):
    """Blue agent that deploys decoys then falls back to react-restore."""

    def __init__(self) -> None:
        super().__init__()
        self._base_decoy_order = np.array([3, 7, 2, 1, 7, 7, 1, 7, 1, 1])
        self.decoy_order: np.ndarray = self._base_decoy_order.copy()
        self.obs_set: int | None = None

    def reset(self) -> None:
        self.decoy_order = self._base_decoy_order.copy()
        self.obs_set = None
        super().reset()
    ...
    def get_action(self, observation: np.ndarray) -> np.ndarray:
        if self.obs_set is None:
            ...
            self.decoy_order = np.tile(
                self._base_decoy_order.reshape(1, -1), (self.obs_set, 1))
